...
# Configuration API
API_ENDPOINT = "http://192.168.15.63:5000/data"
API_KEY = ""
...
def calculate_air_quality_score(co_ppm, co2_ppm, temp, hum):
    ...
    # Calcul note global
    total_score = (co_score * 0.4) + (co2_score * 0.4) + \
        (temp_score * 0.1) + (hum_score * 0.1)
...
def create_json_payload(co_ppm, co2_ppm, temp, hum, air_quality_score, quality_description, motion):
...
try:
    while True:
        ...
        payload = create_json_payload(
            co_ppm, co2_ppm, temp, hum, air_quality_score, quality_description, motion_detected
        )
        ...
        print(f"  • Mouvement: {'Oui' if motion_detected else 'Non'}")
        ...
        if wifi_connected:
            ...
            print(
                f"Prochaine tentative de connexion dans {int((WIFI_RETRY_INTERVAL - (current_time - last_wifi_attempt)) / 60)} minutes")
